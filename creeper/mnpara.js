...
let url = 'http://api.tianapi.com/txapi/mnpara/index?';
let count = 0;
let checkTime = 100;
function get(url) {
  superagent.get(url)
  .end(function (err, res) {
    ...
    let lastData = `SELECT id FROM mnpara ORDER BY id desc limit 1`;
    connection.query(lastData, function(errl, resl) {
      ...
      let sql = `SELECT content FROM mnpara WHERE content = "${result.newslist[0].content}"`;
      connection.query(sql, function (errq, resq) {
        if(resq && resq.length < 1){
          console.log(`次数: ${count} 唯一`);
          let addSql = 'INSERT INTO mnpara(author, content, create_time) VALUES(?, ?, ?)';
          let addSqlParams = [
            result.newslist[0].author,
