...
let url = 'http://api.tianapi.com/txapi/pyqwenan/index?';
let count = 0;
function get(url) {
  superagent.get(url)
    .end(function (err, res) {
      let result = JSON.parse(res.text);
      let sql = 'SELECT content FROM pyqwenan';
      connection.query(sql, function (errq, resq) {
        ...
        if(index == -1){
          let addSql = 'INSERT INTO pyqwenan(source, content, create_time) VALUES(?, ?, ?)';
          let addSqlParams = [
